...
3. **Don't poll on the client when the relay already waited.** The `/api/relay` route does its own `waitForTransactionReceipt({ hash, timeout: 60_000 })` server-side BEFORE returning the hash. By the time the client gets the hash back, the tx is on-chain and confirmed. Re-polling on the client is wasted RPC requests AND fails when public RPCs return 403.

**The April 27 evening fix (relay returns event data):** `/api/relay/route.ts` now decodes `OrderPlaced`, `ListingCreated`, and `SellerRegistered` events server-side and returns the IDs alongside the transaction hash. The gasless hooks (`useGaslessTransaction.ts`, `usePrivyGaslessTransaction.ts`) return a structured `GaslessResult = { hash; orderId?; listingId?; sellerId? }` instead of just a hash string. This lets every gasless write hook skip the client-side `waitForTransactionReceipt` entirely and read the IDs directly from the relay response. `useCreateListing` was the first to use this pattern (post Apr 27 ~5pm fix). `usePurchase`, `useOrderActions`, and `useAdminActions` were patched the same evening.

**Reference plan + audit (Apr 27 2026 evening):** `~/.claude/plans/localroots-payment-bugs-audit.md` and `~/.claude/plans/localroots-payment-bugs-fix-plan.md` document the 6-sub-phase fix. Sequencing: 5 (relay event decode) → 1 (usePurchase + executeGasless return type) → 2 (useTokenApproval timeout + allowance fallback) → 3 (gaslessSuccess pattern on 5 seller hooks) → 4 (wagmi supportedChains reorder) → 6 (`NEXT_PUBLIC_RPC_URL` paid Coinbase Cloud RPC, biggest reliability win).

**Pattern for any new gasless write hook:** call `executeGasless(...)`, receive `GaslessResult | null`, treat success as truthy result. Read `result.hash`, `result.orderId`, etc. as needed. NEVER add client-side `waitForTransactionReceipt` for the gasless path — the relay already waited. Track success synchronously when the hash arrives via the `useCreateListing.ts:53-97` pattern (split state for gasless vs non-gasless, `gaslessSuccess` boolean keyed off `gaslessTxHash`, only call `useWaitForTransactionReceipt` for non-gasless paths with explicit `chainId: ACTIVE_CHAIN_ID`).
